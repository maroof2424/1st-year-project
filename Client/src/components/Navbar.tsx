          ...
          <AnimatePresence>
            {activeMenu === key && (
              <motion.ul
                className={
                  mobile
                    ? "ml-4 mt-2 space-y-2 border-l border-gray-700 pl-4"
                    : "absolute top-12 left-1/2 -translate-x-1/2 bg-gray-800 shadow-2xl rounded-xl px-6 py-4 min-w-[220px] border border-gray-700 space-y-2"
                }
              ...
              >
                {Object.entries(items).map(([item]) => (
                  <li key={item} onMouseEnter={() => !mobile && setActiveSub(item)}>
                    <Link
      ...
      <nav className="fixed top-0 left-0 w-full bg-gray-950 px-6 py-4 z-50 shadow-md">
        <div className="max-w-7xl mx-auto flex items-center justify-between relative">
          <Link to="/" onClick={closeAll}>
            <h1 className="text-white text-xl sm:text-2xl font-bold hover:text-blue-400">
              CodePilot
            </h1>
